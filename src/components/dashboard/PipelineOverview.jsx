...
const PipelineOverview = ({ leads }) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 p-6 h-full flex flex-col justify-center">
      <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-200 mb-6">Pipeline Overview</h3>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {statuses.map(status => {
          const count = getStatusCount(status.label);
          return (
            <div key={status.label} className="flex flex-col items-center p-3 bg-slate-50 dark:bg-slate-900 rounded-lg">
              <div className="flex items-center gap-2 mb-1">
                <span className={`w-3 h-3 rounded-full ${status.color}`} />
                <span className="text-sm font-medium text-slate-600 dark:text-slate-400">{status.label}</span>
              </div>
              <span className="text-2xl font-bold text-slate-800 dark:text-slate-200">{count}</span>
            </div>
          );
        })}
      </div>
